...
//   const sketch = useCallback((s: any) => {
//     // ... original sketch body unchanged ...
...
//     function deriv(t1: number, t2: number, om1: number, om2: number, r1: number, r2: number) {
//       const delta = t2 - t1;
//       const den1 = (m1 + m2) * r1 - m2 * r1 * Math.pow(s.cos(delta), 2);
//       const den2 = (r2 / r1) * den1;
//       const om1dot =
//         (m2 * r1 * om1 * om1 * s.sin(delta) * s.cos(delta) +
//           m2 * g * s.sin(t2) * s.cos(delta) +
//           m2 * r2 * om2 * om2 * s.sin(delta) -
//           (m1 + m2) * g * s.sin(t1)) /
//         den1;
//       const om2dot =
//         (-m2 * r2 * om2 * om2 * s.sin(delta) * s.cos(delta) +
//           (m1 + m2) * (g * s.sin(t1) * s.cos(delta) - r1 * om1 * om1 * s.sin(delta) - g * s.sin(t2))) /
//         den2;
//       return [om1, om2, om1dot, om2dot];
//     }

//     function rk4(state: number[], dt: number, r1: number, r2: number) {
//       const k1 = deriv(state[0], state[1], state[2], state[3], r1, r2);
//       const k2 = deriv(
//         state[0] + k1[0] * dt * 0.5,
//         state[1] + k1[1] * dt * 0.5,
...
//      // buffer も同サイズにリサイズし、行列をリセットして原点移動
//       buffer.resizeCanvas(s.width, s.height);
//       buffer.resetMatrix();
//       buffer.background(0);
//       buffer.translate(cx, cy);
//     };
